...
const PostService = {
  getFeed: async (
    ...
    try {
      ...
      return {
        success: false,
        error:
          error.response?.data?.error || 'Не удалось загрузить ленту постов',
      };
    }
  },
  ...
  getPost: async (postId: string): Promise<ApiResponse> => {
    try {
      ...
      return {
        success: false,
        error: error.response?.data?.error || 'Не удалось загрузить пост',
      };
    }
  },
  ...
  createPost: async (postData: PostData | FormData): Promise<ApiResponse> => {
    try {
      ...
      if (!(postData instanceof FormData) && typeof postData === 'object') {
        ...
        if (
          postData.music &&
          Array.isArray(postData.music) &&
          postData.music.length > 0
        ) {
  ...
  likePost: async (postId: string): Promise<ApiResponse> => {
    try {
      ...
      return {
        success: false,
        error: error.response?.data?.error || 'Не удалось поставить лайк',
      };
    }
  },
    ...
    try {
      ...
      return {
        ...
        error:
          ...
          'Не удалось удалить пост. Проверьте соединение и попробуйте снова.',
      };
    }
  },
  ...
  refreshFeed: async (): Promise<ApiResponse> => {
    try {
      ...
      if (typeof window !== 'undefined') {
        window.dispatchEvent(
          new CustomEvent('feed-refreshed', {
