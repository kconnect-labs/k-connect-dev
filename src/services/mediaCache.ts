...
class MediaCacheService {
  ...
  private async queueCacheOperation(
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onupgradeneeded = event => {
        ...
        if (!db.objectStoreNames.contains(this.storeName)) {
          const mediaStore = db.createObjectStore(this.storeName, {
            keyPath: 'url',
          });
              ...
              element.addEventListener('load', () => {
                if (self.canPerformCacheOperation()) {
                  self.queueCacheOperation(async () => {
                    try {
                      await self.cacheImageFromElement(
  ...
  private isPriorityMediaFile(url: string): boolean {
    ...
    const hasPriorityExtension = priorityExtensions.some(ext =>
      lowerUrl.includes(ext)
    );

    // Only cache from specific high-priority folders
    const isPriorityFolder =
      lowerUrl.includes('/uploads/avatar/') ||
  ...
  private async compressImage(blob: Blob, url: string): Promise<Blob> {
    if (
      !this.imageCompressionSettings.enableCompression ||
      !this.isImageFile(url)
    ) {
      return blob;
    }
  ...
  private async compressImageNonBlocking(
    blob: Blob,
    url: string
  ): Promise<Blob> {
  ...
  private async cacheImageFromElement(
  ...
  async set(url: string, blob: Blob): Promise<void> {
    ...
    const cachedMedia: CachedMedia = {
      ...
      originalSize:
        originalSize !== compressedBlob.size ? originalSize : undefined,
    };
  ...
  async getStats(): Promise<CacheStats> {
    ...
    return new Promise((resolve, reject) => {
      ...
      request.onsuccess = () => {
        resolve(
          request.result || { totalSize: 0, itemCount: 0, lastCleanup: 0 }
        );
      };
    });
  }
  ...
  updateCompressionSettings(
    settings: Partial<typeof this.imageCompressionSettings>
  ): void {
...
export const preloadMediaUrls = (urls: string[]) =>
  mediaCacheService.preloadUrls(urls);
export const getAllCachedMedia = () => mediaCacheService.getAllCachedMedia();

export const updateImageCompressionSettings = (settings: any) =>
  mediaCacheService.updateCompressionSettings(settings);
export const getImageCompressionSettings = () =>
  mediaCacheService.getCompressionSettings();
export const recompressAllImages = () =>
  mediaCacheService.recompressAllImages();
...
export const setMediaCachePerformanceMode = (enabled: boolean) =>
  mediaCacheService.setPerformanceMode(enabled);
