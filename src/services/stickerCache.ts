...
class StickerCacheService {
  ...
  private async preloadWithLowPriority(
    stickersToLoad: string[]
  ): Promise<void> {
    const batchSize = 5;
        ...
        batch.map(async (url, index) => {
          if (index > 0) {
            await new Promise(resolve =>
              setTimeout(resolve, delayBetweenStickers)
            );
          }
  ...
  private async preloadFromCompressed(): Promise<void> {
    try {
      ...
      if (!userId) {
        console.warn(
          'No user ID available for compressed loading, falling back to regular preloading'
        );
      ...
      const response = await fetch(
        `${this.API_URL}/messenger/stickers-compressed/${userId}`,
        {
          headers,
        }
      );
      ...
      if (data.success && data.stickers) {
        console.log(
          `Compressed API loaded: ${data.stickers.length} static stickers`
        );
        ...
        for (const sticker of data.stickers) {
          try {
            ...
            console.log(
              `Compressed cached static sticker: ${sticker.name} (${sticker.mime_type})`
            );
          } catch (error) {
            console.warn(
              `Failed to process compressed sticker ${sticker.id}:`,
              error
            );
          }
        }
  ...
  private async preloadFromSimpleBatch(): Promise<void> {
    try {
      ...
      const response = await fetch(
        `${this.API_URL}/messenger/stickers-batch-simple/${userId}`,
        {
          headers,
        }
      );
      ...
      if (data.success && data.stickers) {
        console.log(
          `✅ Simple batch API loaded: ${data.stickers.length} stickers`
        );
        ...
        for (const sticker of data.stickers) {
          try {
            console.log(
              `🔄 Processing sticker ${sticker.id} (${sticker.mime_type})`
            );
            ...
            console.log(
              `✅ Cached sticker: ${sticker.name} (${sticker.mime_type}) - ${sticker.file_size} bytes`
            );
            ...
            if (
              ...
              console.log(
                `🎯 Successfully cached problematic sticker ${sticker.id}:`,
                {
            ...
            console.error(
              `❌ Failed to process simple batch sticker ${sticker.id}:`,
              error
            );
          }
        }
  ...
  private async extractArchive(
    archiveBlob: Blob
  ): Promise<{ metadata: any; files: Record<string, Uint8Array> }> {
    return new Promise((resolve, reject) => {
      reject(
        new Error(
          'Archive extraction not implemented yet, falling back to regular loading'
        )
      );
    });
  }
  ...
  private cleanup(): void {
    ...
    if (entriesToDelete.length > 0) {
      console.log(
        `Cleaned up ${entriesToDelete.length} expired sticker cache entries`
      );
      this.updateStats();
      this.saveToStorage();
    }
...
export const useStickerCache = () => {
  const getSticker = React.useCallback(
    (url: string) => stickerCacheService.getSticker(url),
    []
  );

  const isCached = React.useCallback(
    (url: string) => stickerCacheService.isCached(url),
    []
  );

  const getStats = React.useCallback(() => stickerCacheService.getStats(), []);
