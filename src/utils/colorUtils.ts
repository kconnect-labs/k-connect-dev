 ...
 */
export function hexToLottieRgba(
  hex: string,
  alpha: number = 1
 ...
 */
export function rgbToLottieRgba(
  rgb: string,
  alpha: number = 1
): [number, number, number, number] {
 ...
 */
export function createContrastColor(
  color: [number, number, number, number],
  factor: number = 0.2
): [number, number, number, number] {
  const [r, g, b, a] = color;
...
export function createProfileColorPalette(profileColor: string): {
  ...
  if (profileColor.startsWith('#')) {
    ...
    baseColor = ORIGINAL_LOTTIE_COLORS.primary as [
      number,
      number,
      number,
      number,
...
export function applyColorPaletteToLottie(
  ...
  function findSimilarColor(
    ...
    for (let i = 0; i < originalColors.length; i++) {
      if (colorsEqual(color, originalColors[i], 0.001)) {
        console.log(
          `🎨 Found exact match for color ${JSON.stringify(color)} -> ${JSON.stringify(newColors[i])}`
        );
        return newColors[i];
      }
    }
    ...
    for (let i = 0; i < originalColors.length; i++) {
      if (colorsEqual(color, originalColors[i], 0.1)) {
        console.log(
          `🎨 Found approximate match for color ${JSON.stringify(color)} -> ${JSON.stringify(newColors[i])}`
        );
        return newColors[i];
      }
    }
    ...
    if (brightness < 0.3) {
      ...
      console.log(
        `🎨 Dark color ${JSON.stringify(color)} replaced with stroke color`
      );
      return colorPalette.stroke;
    } else if (brightness > 0.7) {
      // Светлые цвета заменяем на secondary
      console.log(
        `🎨 Light color ${JSON.stringify(color)} replaced with secondary color`
      );
      return colorPalette.secondary;
    } else {
      // Остальные цвета заменяем на primary
      console.log(
        `🎨 Medium color ${JSON.stringify(color)} replaced with primary color`
      );
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (key === 'c' && Array.isArray(value) && value.length === 4) {
          ...
          if (newColor) {
            colorReplacements++;
            console.log(
              `🎨 Replacing color ${JSON.stringify(value)} with ${JSON.stringify(newColor)}`
            );
            newObj[key] = newColor;
          } else {
            newObj[key] = value;
  ...
  console.log(
    `🎨 applyColorPaletteToLottie: Replaced ${colorReplacements} colors`
  );
  return result;
}
 ...
 */
export function createAdaptiveColorPalette(
  user: any
): ReturnType<typeof createProfileColorPalette> | null {
