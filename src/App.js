...
import {getLightTheme} from './UIKIT/themes/lightTheme'
import {getDarkTheme} from './UIKIT/themes/darkTheme'
			...
			try {
				...
				broadcastChannel.current.onmessage = event => {
					...
					if (type === 'session_refresh') {
						...
						type === 'last_fetch_update' &&
						timestamp > (lastFetchTime.current || 0)
					) {
...
function App() {
	...
	const saveUserBackground = url => {
		if (url) {
			...
			document.cookie = `myProfileBackgroundUrl=${encodeURIComponent(
				url
  ...
  const themeMap = {
		light: getLightTheme(themeSettings.enableBlur),
		dark: getDarkTheme(themeSettings.enableBlur),
		// ...другие темы
	}

	const theme = useMemo(() => {
		// 1. Выбираем базовую тему
		const base = themeMap.dark

		// 2. Собираем переопределения из themeSettings
		const overrides = {
			palette: {
				...
				...(themeSettings.backgroundColor && {
					background: {
						...
						paper: themeSettings.backgroundColor,
					},
				}),
				...(themeSettings.textColor && {
					text: { primary: themeSettings.textColor },
				}),
			},
			shape: {
			...
			components: {
				MuiPaper: {
					styleOverrides: {
						root: {
							...(themeSettings.enableBlur === false
								? { backdropFilter: 'none' }
		...
		// 3. Объединяем базовую тему и overrides
		const mergedTheme = createTheme(deepmerge(base, overrides))
