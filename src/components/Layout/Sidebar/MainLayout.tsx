...
const MemoizedSidebar = memo<MemoizedSidebarProps>(
  ({ open, onClose, isMobile, layoutVersion }) => {
    if (isMobile) {
      return null;
    }
    
    // Выбираем версию сайдбара в зависимости от layoutVersion
    if (layoutVersion === 'v2') {
      // @ts-ignore
      return <SidebarV2 isMobile={isMobile} />;
    }
...
const MainLayout: React.FC<MainLayoutProps> = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const handleLayoutVersionChange = (
      event: CustomEvent<{ version: string }>
    ) => {
      const { version } = event.detail;
      console.log('MainLayout received version change:', version);
  ...
  // Рендер V1 (текущая версия)
  const renderV1Layout = () => (
    <MainContainer
    ...
    >
      ...
      {!isMobile && hasDesktopPlayer && <DesktopPlayer isMobile={false} />}
  ...
  // Рендер V2 (новая версия - пока идентична V1)
  const renderV2Layout = () => (
    <MainContainer
      data-testid="main-container"
      className={hasBackgroundImage ? '' : 'theme-site-background'}
      sx={{
  ...
  // Отладка текущей версии
  console.log('MainLayout rendering with version:', layoutVersion);
  
  // Возвращаем нужную версию в зависимости от выбора
  return layoutVersion === 'v2' ? renderV2Layout() : renderV1Layout();
};
