...
const RepostCard = styled(Card)(({ theme, background }) => ({
  ...
  backgroundImage: background
    ? `linear-gradient(rgba(120, 100, 160, 0.03), rgba(20, 20, 20, 0.95)), url(${background})`
...
const CardOverlay = styled(Box)(({ theme }) => ({
  ...
  borderRadius: '12px',
  background:
    'linear-gradient(145deg, rgba(30, 30, 30, 0.9), rgba(26, 26, 26, 0.97))',
  backdropFilter: 'blur(15px)',
  zIndex: 0,
}));
...
const RepostItem = ({ repost, onDelete, onOpenLightbox }) => {
  ...
  React.useEffect(() => {
    if (originalPost.music) {
      try {
        const parsedTracks =
          typeof originalPost.music === 'string'
            ? JSON.parse(originalPost.music)
            : originalPost.music;
          ...
          <OriginalPostCard>
            ...
            >
              <Box sx={{ display: 'flex', alignItems: 'center', mr: 2 }}>
                <FavoriteIcon
                  sx={{ fontSize: 16, mr: 0.5, color: '#e57373' }}
                />
                <Typography variant='caption' color='text.secondary'>
