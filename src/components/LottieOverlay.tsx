...
const LottieOverlay: React.FC<LottieOverlayProps> = React.memo(({ 
  ...
  useEffect(() => {
    const loadLottieData = async () => {
      ...
      const isJsonFile = item.item_type?.toLowerCase() === 'json' || 
      ...
      try {
        ...
        if (typeof data === 'string' && data.includes('<!DOCTYPE html>')) {
          throw new Error('Server returned HTML instead of JSON - file not found');
        }
        
        if (!data.assets || !Array.isArray(data.assets)) {
          throw new Error('Invalid Lottie data structure: missing or invalid assets array');
        }
  ...
  return (
    ...
    >
      <Suspense fallback={<CircularProgress size={24} />}>
        {lottieData && lottieData.assets && Array.isArray(lottieData.assets) ? (
          <ErrorBoundary
            fallback={
              <Box
                sx={{
          ...
          >
             <LottiePlayer
               ...
               loop={false}
               play={true}
               speed={performanceMode === 'low' ? 0.5 : performanceMode === 'medium' ? 0.8 : 1}
          ...
          >
            {isLoading ? 'Загрузка...' : hasError ? 'Ошибка загрузки' : 'Нет данных'}
          </Box>
        )}
      </Suspense>
    </div>
