...
const DecorationCard = styled(motion.div)(({ theme, selected }) => ({
  ...
  overflow: 'hidden',
  cursor: 'pointer',
  border: selected ? '2px solid rgba(255, 255, 255, 0.3)' : '1px solid rgba(255, 255, 255, 0.12)',
...
const SelectionOverlay = styled(Box)(({ theme, selected }) => ({
  ...
  height: 24,
  borderRadius: '50%',
  background: selected ? 'rgba(255, 255, 255, 0.9)' : 'rgba(255, 255, 255, 0.1)',
  display: 'flex',
  alignItems: 'center',
...
const UltimateDecorationModal = ({ 
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Произошла ошибка при выборе декорации');
    } finally {
      setIsSubmitting(false);
    }
  };
        ...
        {success ? (
          <SuccessContent>
            ...
            <Typography variant="body1" color="text.secondary" paragraph>
              Декорация "{selectedDecoration?.name}" успешно применена к вашему профилю
            </Typography>
          ...
          <Box sx={{ 
            ...
            <Typography variant="body1" paragraph sx={{ 
              ...
              Выберите одну из доступных декораций для вашего профиля. 
              Декорация будет применена немедленно и останется активной навсегда.
              ...
              <AnimatePresence>
                {availableDecorations.map((decoration, index) => (
                  ...
                  >
                    ...
                    >
                      <SelectionOverlay selected={selectedDecoration?.id === decoration.id}>
                        {selectedDecoration?.id === decoration.id && <CheckCircleIcon />}
                      </SelectionOverlay>
                      
                      <DecorationPreview background={decoration.background ? (
                        decoration.background.includes('/') ? `url(${API_URL}/${decoration.background})` : decoration.background
                      ) : undefined}>
                        {decoration.item_path && (() => {
                          const { path, styles } = parseItemSettings(decoration.item_path);
                          return (
                            <DecorationItem 
                              src={`${API_URL}/${path}`}
                              alt={decoration.name}
