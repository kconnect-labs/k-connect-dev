...
// Импорт хуков
import { useFullScreenPlayer, usePortal } from './hooks';

// Импорт типов
import { FullScreenPlayerProps, PlayerTrackInfoProps } from './types';
...
const HeaderSection = memo(styled(Box)`
  display: flex;
  align-items: center;
...
const CloseButton = memo(styled(IconButton)`
...
const AlbumArtContainer = memo(styled(Box)`
  display: flex;
...
const AlbumArt = memo(styled.img`
...
const TrackInfo = memo(styled(Box)`
  text-align: center;
  max-width: 600px;
  margin-bottom: 16px;
  flex: 0 0 auto;
...
const TrackTitle = memo(styled(Typography)`
  font-size: 1.5rem;
  font-weight: 700;
  color: white;
  margin-bottom: 6px;
...
const TrackArtist = memo(styled(Typography)`
  font-size: 0.9rem;
...
const ProgressContainer = memo(styled(Box)`
  width: 100%;
  display: flex;
...
const TimeDisplay = memo(styled(Box)`
  display: flex;
  justify-content: space-between;
...
const MainControls = memo(styled(Box)`
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 36px;
...
const SecondaryControls = memo(styled(Box)`
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 36px;
...
const PlayerTrackInfo: React.FC<PlayerTrackInfoProps> = memo(
  ({ currentTrack, onArtistClick }) => {
...
// Основной компонент
const FullScreenPlayerCore: React.FC<FullScreenPlayerProps> = memo(
  ({ open, onClose }) => {
              ...
              >
                <Box
                  sx={{
                    ...
                    minHeight:
                      isMobile && lyricsDisplayMode ? '300px' : '350px',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    overflow: 'hidden',
                  ...
                  (lyricsData?.has_synced_lyrics || lyricsData?.lyrics) ? (
                    <Box
                      sx={{
                        ...
                        transition: 'all 0.6s cubic-bezier(0.4, 0, 0.2, 1)',
                        animation:
                          'fadeInScale 0.6s cubic-bezier(0.4, 0, 0.2, 1)',
                ...
                >
                      ...
                      sx={{
                        ...
                        '&:hover': {
                          '& .MuiSlider-track': {
                            height: 14,
                            borderRadius:
                              'var(--large-border-radius)!important',
                          },
                          '& .MuiSlider-rail': {
                            height: 14,
                            borderRadius:
                  ...
                  <SecondaryControls>
                    <ControlButton
                      ...
                      className='secondary'
                      sx={{
                        color: (currentTrack as any)?.is_liked
                          ? '#ff2d55'
                    ...
                    {(lyricsData?.has_synced_lyrics || lyricsData?.lyrics) && (
                      <ControlButton
                        ...
                        className='secondary'
                        sx={{
                          color: lyricsDisplayMode
                            ? '#9a7ace'
                            : 'rgba(255,255,255,0.8)',
                          '&:hover': {
                            color: lyricsDisplayMode ? '#9a7ace' : 'white',
                          },
                        }}
                      >
                    ...
                    <ControlButton
                      ...
                      sx={{
                        color: showLyricsEditor
                          ...
                          : 'rgba(255,255,255,0.8)',
                        '&:hover': {
                          color: showLyricsEditor ? '#9a7ace' : 'white',
                        },
                      }}
                    >
            ...
            <CloseButton onClick={onClose} disableRipple>
              ...
              >
                <path
                  fill-rule='evenodd'
                  clip-rule='evenodd'
                  d='M29.9049 2.44378C30.446 2.04778 30.4605 1.23555 29.9251 0.8319L29.4148 0.44717C29.0626 0.181672 28.5782 0.178211 28.2222 0.438652L16.0701 9.3312C15.7185 9.58849 15.2408 9.58852 14.8892 9.33128L14.0566 8.72226L14.0645 8.72745L2.80182 0.484793C2.45012 0.227397 1.97222 0.227426 1.62055 0.484863L1.1024 0.864167C0.556606 1.26371 0.555576 2.07777 1.10142 2.47725C4.73609 5.13731 11.7446 10.2664 14.8914 12.5694C15.2429 12.8267 15.72 12.8243 16.0716 12.5671C17.4436 11.5638 17.174 11.7611 29.9049 2.44378Z'
                  fill='white'
                />
              </svg>
            </CloseButton>
...
// Компонент портала
const FullScreenPlayerPortal: React.FC<FullScreenPlayerProps> = memo(
  ({ open, onClose }) => {
...
const StaticLyricsLine: React.FC<{
  text: string;
  index: number;
  isMainDisplay?: boolean;
}> = memo(({ text, index, isMainDisplay }) => (
  <Typography
    variant='h5'
    className='lyrics-line'
    sx={{
      ...
      <Box sx={{ width: '100%', zIndex: 99000 }}>
        ...
        <Menu
          ...
          PaperProps={{
            style: {
              backgroundColor:
                'var(--theme-background, rgba(25, 25, 25, 0.95))',
