...
import {
  Dialog, DialogTitle, DialogContent, DialogActions, Button, TextField, Typography, Box, Alert, CircularProgress, FormControlLabel, Checkbox, List, ListItem
} from '@mui/material';
  ...
  return (
    ...
    >
          ...
          <Box sx={{ mt: 2, mb: 1 }}>
            ...
            <List dense>
              {post.music.map((track, idx) => (
                <ListItem key={`track-${idx}`}>{track.title || t('post.edit_dialog.audio_track')}</ListItem>
              ))}
            </List>
            <FormControlLabel
              control={
      ...
      <DialogActions sx={{ p: 2, justifyContent: 'space-between' }}>
        ...
        >
          {submitting ? t('post.edit_dialog.submitting') : t('post.edit_dialog.save')}
        </Button>
      </DialogActions>
    </Dialog>
  );
