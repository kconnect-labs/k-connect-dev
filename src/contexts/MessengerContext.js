...
const formatToLocalTime = isoDateString => {
  ...
  try {
    ...
    console.error(
      'formatToLocalTime: Ошибка преобразования времени:',
      e,
      isoDateString
    );
    return typeof isoDateString === 'string' ? isoDateString : '';
  ...
  const sendTextMessage = async (chatId, text, replyToId = null) => {
    ...
    try {
      ...
      if (websocketClient.current && websocketClient.current.isConnected) {
        ...
        websocketClient.current.sendChatMessage(
          chatId,
          messageText,
          replyToId,
          tempMessage.id
...
export const useMessenger = () => {
  ...
  if (!context) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.warn(
        'useMessenger: MessengerProvider недоступен. Возможно, компонент рендерится до инициализации провайдера.'
      );
  ...
  if (!context.isProviderReady && process.env.NODE_ENV === 'development') {
    console.warn(
      'useMessenger: MessengerProvider еще не готов, возвращаем базовые значения'
    );
  }
