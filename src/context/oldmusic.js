...
export const MusicProvider = ({ children }) => {
  ...
  useEffect(() => {
    ...
    const fetchTracks = async () => {
      if (
        ...
        console.log(
          `Уже есть загруженные треки для секции: ${currentSection}, пропускаем загрузку`
        );
        return;
      }
      ...
      if (
        ...
        if (likedTracks.length === 0 && cacheAge < 15 * 60 * 1000) {
          console.log(
            'Уже проверили, что нет лайкнутых треков, пропускаем запрос (кеш действителен 15 минут)'
          );
          return;
        }
        ...
        if (likedTracks.length > 0 && cacheAge < 5 * 60 * 1000) {
          console.log(
            'Используем кешированные лайкнутые треки (кеш действителен 5 минут)'
          );
          return;
        }
      }
      ...
      console.log(
        `Загрузка треков. isLoadingRef.current=${isLoadingRef.current}`
      );
      ...
      try {
        ...
        if (currentSection === 'liked') {
          ...
          if (isLoadingLikedTracksRef.current) {
            console.log(
              'Загрузка понравившихся треков уже идет, пропускаем повторный запрос'
            );
            isLoadingRef.current = false;
            setIsLoading(false);
            return;
          ...
          try {
            ...
            if (response.data && response.data.tracks) {
              ...
              console.log(
                `Получено ${receivedTracks.length} понравившихся треков`
              );
        ...
        if (response.data && response.data.tracks) {
          ...
          if (receivedTracks.length > 0) {
            ...
            if (isPlaying || currentTrack) {
              ...
              console.log(
                `Pre-fetching lyrics for first ${prefetchCount} tracks...`
              );
          ...
          if (currentSection === 'all') {
            ...
            console.log(
              'Загружены и перемешаны случайные треки:',
          ...
          setTimeout(() => {
            if (
              ...
              console.log(
                `Предзагрузка следующей порции треков для секции ${currentSection}...`
              );
  ...
  const playTrack = (track, section = currentSection) => {
    ...
    try {
      ...
      console.log(
        `Начинаем воспроизведение трека: ${track.title} - ${track.artist}`
      );
  ...
  const getNextTrack = async () => {
    try {
      ...
      if (currentSection && currentSection.startsWith('playlist_')) {
        ...
        const currentIndex = tracksInPlaylist.findIndex(
          track => track.id === currentTrack.id
        );

        if (currentIndex === -1) {
          console.log(
            'Current track not found in playlist, returning first track'
          );
      ...
      if (
        ...
        console.log(
          `Trigger pagination: ${isNearingEnd ? 'nearing end of list' : 'approaching track at multiple of 15'}`
        );
        console.log(
          `Current index: ${currentIndex}, Total tracks in list: ${trackList.length}`
        );
      ...
      if (currentIndex < trackList.length - 1) {
        ...
        if (hasMoreByType[currentSection]) {
          console.log(
            'Reached last track, attempting to load more before looping'
          );
  ...
  const getPreviousTrack = async () => {
    try {
      ...
      if (currentSection && currentSection.startsWith('playlist_')) {
        ...
        if (currentIndex === -1) {
          console.log(
            'Current track not found in playlist, returning last track'
          );
      ...
      const isNearing15Increment =
        currentIndex % 15 <= 2 || currentIndex % 15 === 14;
      ...
      if (
        ...
        console.log(
          `Trigger pagination backward: ${isNearingStart ? 'nearing start of list' : 'approaching track at multiple of 15'}`
        );
        console.log(
          `Current index: ${currentIndex}, Total tracks in list: ${trackList.length}`
        );
  ...
  const likeTrack = async trackId => {
    try {
      ...
      if (response.data.success) {
        ...
        console.log(
          `Track ${trackId} ${isLiked ? 'liked' : 'unliked'} successfully`
        );
        ...
        const extractMetadata = () => {
          return new Promise((resolve, reject) => {
            if (typeof window.jsmediatags === 'undefined') {
              console.log(
                'jsmediatags не загружен, пропускаем извлечение метаданных'
              );
              resolve(null);
              return;
            }
  ...
  useEffect(() => {
    ...
    if ('mediaSession' in navigator) {
      ...
      if (isIOS) {
        console.log(
          'iOS device detected, applying special handling for media controls'
        );
  ...
  const loadMoreTracks = useCallback(
    async (type = 'all') => {
      ...
      if (isLoadingMoreRef.current) {
        console.log(
          `[loadMoreTracks] Пропускаем загрузку для ${type}: уже идет загрузка`
        );
        return false;
      }
      ...
      if (!hasMoreForType) {
        console.log(
          `[loadMoreTracks] Пропускаем загрузку для ${type}: больше нет треков (hasMoreByType = ${hasMoreByType[type]})`
        );
        return false;
      }
      ...
      if (
        ...
        console.log(
          `[loadMoreTracks] Пропускаем загрузку для ${type}: ожидается загрузка первой страницы`
        );
        return false;
      }

      console.log(
        `[loadMoreTracks] Устанавливаем флаги загрузки для типа: ${type}`
      );
      ...
      try {
        ...
        console.log(
          `[loadMoreTracks] Загрузка страницы ${nextPage} для типа: ${type}`
        );
        ...
        if (type === 'liked') {
          ...
          console.log(
            `[loadMoreTracks] Запрос для понравившихся треков: ${endpoint}`,
            params
          );
        ...
        console.log(
          `[loadMoreTracks] Загружаем треки для типа "${type}" со страницы ${nextPage}`
        );
        ...
        if (response && response.data && response.data.tracks) {
          ...
          console.log(
            `[loadMoreTracks] Данные получены в формате { tracks: [...] }, количество: ${newTracks.length}, has_more: ${has_more}`
          );

          if (newTracks.length > 0) {
            const prefetchCount = Math.min(2, newTracks.length);
            console.log(
              `[loadMoreTracks] Pre-fetching lyrics for first ${prefetchCount} tracks in this batch...`
            );
          ...
          has_more = newTracks.length >= 50;
          console.log(
            `[loadMoreTracks] Данные получены в формате массива, количество: ${newTracks.length}, has_more: ${has_more}`
          );
        } else if (type !== 'liked') {
          console.warn(
        ...
        if (newTracks.length === 0) {
          has_more = false;
          console.log(
            `[loadMoreTracks] Не получено новых треков, устанавливаем has_more = false`
          );
        }

        console.log(
          `[loadMoreTracks] Получено ${newTracks.length} треков для типа ${type}, есть еще: ${has_more}`
        );
        ...
        if (type === 'all') {
          ...
          if (response && response.data && response.data.tracks) {
            console.log(
              `[loadMoreTracks] Добавляем ${response.data.tracks.length} понравившихся треков к существующим ${likedTracks.length}`
            );
            ...
            console.log(
              `[loadMoreTracks] Добавляем ${newTracks.length} понравившихся треков к существующим ${likedTracks.length}`
            );
          ...
          console.log(
            '[loadMoreTracks] Добавлены и перемешаны случайные треки:',
            shuffledNewTracks.length
          );
        }
        ...
        console.log(
          `[loadMoreTracks] Обновлена страница для типа ${type} на ${nextPage}`
        );
        ...
        console.log(
          `[loadMoreTracks] Обновлен флаг hasMoreByType[${type}] = ${has_more}`
        );

        if (currentSection === type) {
          setHasMoreTracks(has_more);
          console.log(
            `[loadMoreTracks] Обновлен основной флаг hasMoreTracks = ${has_more}`
          );
        }
        ...
        if (has_more) {
          ...
          console.log(
            `[loadMoreTracks] Запланирована предзагрузка следующей страницы ${nextPage + 1} для типа ${type}`
          );
        }

        console.log(
          `[loadMoreTracks] Успешно завершена загрузка для типа ${type}`
        );
        return true;
      } catch (error) {
        ...
        if (error.response && error.response.status === 404) {
          ...
          console.log(
            `[loadMoreTracks] Получен 404, устанавливаем hasMoreByType[${type}] = false`
          );
        }
        ...
        console.log(
          `[loadMoreTracks] Сброшены флаги загрузки для типа ${type}`
        );
      }
    },
    [
  ...
  const preloadNextPage = useCallback(async (nextPage, type = 'all') => {
    try {
      ...
      if (now - lastPreloadTime < 30000) {
        console.log(
          `Пропускаем предзагрузку для ${type}: прошло менее 30 секунд с последнего запроса`
        );
        return;
      }
      ...
      if (type === 'liked') {
        ...
        try {
          ...
          console.log(
            `Предзагружено ${response.data.tracks?.length || 0} понравившихся треков для страницы ${nextPage}`
          );
        } catch (error) {
      ...
      console.log(
        `Предзагружено ${response.data.tracks.length} треков для страницы ${nextPage} (тип: ${type})`
      );
    } catch (error) {
      console.error('Ошибка при предзагрузке треков:', error);
    }
  ...
  const searchTracks = async query => {
    ...
    try {
      ...
      const processedTracks = Array.isArray(tracks)
        ? tracks.map(track => {
            return {
              ...track,
              ...
              file_path:
                track.file_path ||
                '/static/uploads/system/audio_placeholder.mp3',
              cover_path:
                track.cover_path ||
  ...
  const resetPagination = useCallback(async (type, randomize = false) => {
    console.log(
      `[resetPagination] Сброс пагинации для типа: ${type}, randomize: ${randomize ? 'true' : 'false'}`
    );
      ...
      try {
        ...
        if (type === 'liked') {
          ...
          try {
            ...
            console.log(
              '[resetPagination] Запрашиваем понравившиеся треки',
              config
            );
            ...
            if (response.data && response.data.tracks) {
              const receivedTracks = response.data.tracks;
              console.log(
                `[resetPagination] Получено ${receivedTracks.length} понравившихся треков`
              );

              const hasMore = receivedTracks.length >= params.per_page;
              console.log(
                `[resetPagination] Устанавливаем hasMoreByType[liked] = ${hasMore}`
              );
              ...
              console.log(
                '[resetPagination] В ответе нет понравившихся треков'
              );
              setLikedTracks([]);
              setHasMoreByType(prev => ({ ...prev, liked: false }));
    ...
    if (initialMount.current) {
      try {
        ...
        if (savedTrack) {
          ...
          console.log(
            'Восстановлен последний трек из localStorage:',
            parsedTrack.title
          );
        }
      } catch (error) {
