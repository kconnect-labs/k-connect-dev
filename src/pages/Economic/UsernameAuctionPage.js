...
const UsernameAuctionPage = () => {
    ...
    const checkSessionExpiration = () => {
      ...
      if (sessionDuration >= SESSION_TIMEOUT && !sessionExpired) {
        ...
        setSnackbar({
          open: true,
          message:
            'Сессия устарела. Пожалуйста, обновите страницу для получения актуальных данных.',
          severity: 'warning',
        });
      ...
      updateInterval.current = setInterval(() => {
        if (sessionActive && document.visibilityState === 'visible') {
          ...
          if (
            ...
            currentTime - lastFetchTime.current > MIN_UPDATE_INTERVAL
          ) {
            fetchAuctions(true);
            fetchUserAuctions(true);
          }
  ...
  const handleCreateAuction = async () => {
    ...
    if (!newAuctionData.min_price) {
      ...
      isNaN(newAuctionData.min_price) ||
      parseInt(newAuctionData.min_price) <= 0
    ) {
  ...
  const handlePlaceBid = async () => {
    ...
    try {
      const response = await axios.post(
        `/api/username/auctions/${selectedAuction.id}/bid`,
        {
  ...
  return (
    <Container maxWidth='md' sx={{ py: 4, mb: 10 }}>
      <PageHeader>
        ...
        >
          Участвуйте в аукционах на уникальные юзернеймы или выставляйте свои
          собственные
        </Typography>
      </PageHeader>
      ...
      <Box sx={{ mb: 2 }}>
        ...
        >
          ...
          <Tab
            ...
            label='Мои аукционы'
          />
          <Tab icon={<HistoryIcon />} iconPosition='start' label='Мои ставки' />
          <Tab
            icon={<HistoryIcon />}
            iconPosition='start'
            label='Завершенные'
            ...
            <Box>
              ...
              >
                ...
                <Typography variant='body2'>
                  Теперь при размещении ставки баллы резервируются, а при
                  перебитии возвращаются автоматически. Также появилась
                  возможность досрочного принятия ставки продавцом!
                </Typography>
              </Alert>
      ...
      >
        ...
        <DialogActions sx={{ px: 3, pb: 3 }}>
          ...
          <Button
            ...
            disabled={
              !newAuctionData.username ||
              !newAuctionData.min_price ||
              loadingButtons.create
            }
            startIcon={
              loadingButtons.create ? <CircularProgress size={20} /> : null
            }
          >
            ...
            <Box sx={{ mb: 3, mt: 2 }}>
              ...
              {selectedAuction.status === 'completed' ? (
                ...
                <TextField
                  ...
                  helperText={
                    errors.bidAmount ||
                    `Минимальная ставка: ${parseInt(selectedAuction.current_price || selectedAuction.highest_bid || selectedAuction.min_price) + 1} баллов`
                  }
                  margin='normal'
                  variant='outlined'
                  disabled={
                    loadingButtons.bid || selectedAuction.status === 'completed'
                  }
                  InputProps={{
                    inputProps: {
                      min:
                      ...
                      >
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography variant='body1'>
                            Продавец:{' '}
                            <strong>
                              {detailAuction.seller
                                ? detailAuction.seller.username
                                : 'Unknown'}
                            </strong>
                          </Typography>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography variant='body1'>
                            Минимальная цена:{' '}
                            <strong>{detailAuction.min_price} баллов</strong>
                          </Typography>
                        </Box>
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          <MonetizationOnIcon
                          ...
                          <Typography variant='body1'>
                            ...
                            <strong>
                              {detailAuction.status === 'completed'
                                ? detailAuction.final_price ||
                                  detailAuction.current_price ||
                                  detailAuction.highest_bid ||
                                  detailAuction.min_price
                                : detailAuction.current_price ||
                                  detailAuction.min_price}{' '}
                              баллов
                            </strong>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>
                          ...
                          <Typography variant='body1'>
                            Создан:{' '}
                            <strong>
                              {formatDate(detailAuction.created_at)}
                            </strong>
                          </Typography>
                        </Box>
                        {detailAuction.status === 'completed' ? (
                          <Box sx={{ display: 'flex', alignItems: 'center' }}>
                            ...
                            <Typography variant='body1'>
                              ...
                              <strong>
                                {formatDate(
                                  detailAuction.completed_at ||
                                    detailAuction.end_time
                                )}
                              </strong>
                            </Typography>
                          </Box>
                          ...
                          <>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              ...
                              <Typography variant='body1'>
                                Завершается:{' '}
                                <strong>
                                  {formatDate(detailAuction.end_time)}
                                </strong>
                              </Typography>
                            </Box>
                            <Box sx={{ display: 'flex', alignItems: 'center' }}>
                              ...
                              <Typography variant='body1'>
                                Осталось:{' '}
                                <strong>
                                  {detailAuction.remaining_time_formatted ||
                                    formatTimeRemaining(detailAuction.end_time)}
                                </strong>
                              </Typography>
                            </Box>
                          </>
                        ...
                        >
                          {detailAuction.bids
                            ...
                            .map((bid, index) => {
                              ...
                              const maxBidAmount = Math.max(...detailAuction.bids.map(b => b.amount));
                              const isHighestBid = bid.amount === maxBidAmount;
                              const isMyBid = bid.bidder.id === user?.id;
                              const isWinner =
                                detailAuction.status === 'completed' &&
                                detailAuction.winner_id === bid.bidder.id;
