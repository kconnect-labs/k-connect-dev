...
export const CreateDialog: React.FC<CreateDialogProps> = ({
  ...
  const handleInputChange = (field: keyof NewBadge, value: string | number | boolean) => {
    onNewBadgeChange({
      ...newBadge,
      [field]: value
    });
    ...
    >
      ...
      <DialogContent sx={{ p: isMobile ? 2 : 3, overflow: 'auto' }}>
        <Box sx={{ 
          ...
          <Typography variant="body2" color="text.secondary" sx={{ mb: 2 }}>
            Создано: {createdBadgesCount} / {badgeLimit === Infinity ? '∞' : badgeLimit}
          </Typography>
          ...
          {badgeLimitReached && (
            <Alert severity="warning" sx={{ mb: 2 }}>
              Достигнут лимит создания бейджей ({badgeLimit}). Обновите подписку для увеличения лимита.
            </Alert>
          )}
        ...
        <Box sx={{ 
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <TextField
              ...
              onChange={(e) => handleInputChange('royalty_percentage', Number(e.target.value))}
              fullWidth
              type="number"
            ...
            <Box sx={{ mb: 2, p: 2, bgcolor: 'rgba(208, 188, 255, 0.1)', borderRadius: 1, border: '1px solid rgba(208, 188, 255, 0.3)' }}>
                ...
                <FormControlLabel
                  control={
                    <Switch
                      ...
                      onChange={(e) => handleInputChange('is_upgraded', e.target.checked)}
                      disabled={badgeLimitReached}
                    />
                  }
                  label=""
              ...
              <Typography variant="body2" color="text.secondary">
                Стоимость создания: {newBadge.is_upgraded ? '9000' : '3000'} баллов
              </Typography>
            </Box>
            ...
            {newBadge.is_upgraded && (
              <Box sx={{ mb: 2 }}>
                ...
                <input
                  ...
                  onChange={(e) => handleInputChange('particle_color', e.target.value)}
                  disabled={badgeLimitReached}
                  style={{ 
                    width: '100%', 
                    height: 40, 
            ...
            <Box sx={{ mb: 2 }}>
              ...
              <Box sx={{ mt: 2, p: 2, bgcolor: 'rgba(255, 255, 255, 0.03)', borderRadius: 1 }}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ mb: 1 }}>
                  • Рекомендуем использовать SVG с минимальным весом для лучшей производительности
                </Typography>
              </Box>
            </Box>
          </Box>
      ...
      <DialogActions sx={{ p: 3, pt: 0 }}>
        ...
        <Button
          onClick={onCreate}
          variant="contained"
          disabled={loading || badgeLimitReached || !newBadge.name || !newBadge.description || !newBadge.price || !newBadge.image}
