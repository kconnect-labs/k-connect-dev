...
export const useBadgeShopAPI = ({
  ...
  const updateBadgeLimit = useCallback((subscriptionType: string) => {
    if (!subscriptionType) {
      setBadgeLimit(3);
      return;
    }
  ...
  }, [setCreatedBadgesCount, setBadgeLimitReached, userSubscription, badgeLimit]);

  const createBadge = useCallback(async (newBadge: NewBadge) => {
    try {
      ...
      if (userPoints < creationCost) {
        throw new Error(`Недостаточно баллов. Требуется: ${creationCost} баллов`);
      }
      ...
      formData.append('royalty_percentage', newBadge.royalty_percentage.toString());
      ...
      throw new Error(error.response?.data?.error || error.message || 'Ошибка при создании бейджика');
    }
  }, [userPoints, fetchCreatedBadgesCount, fetchBadges, fetchUserPoints]);

  const purchaseBadge = useCallback(async (badge: Badge) => {
    try {
      ...
      throw new Error(error.response?.data?.error || 'Ошибка при покупке бейджика');
    }
