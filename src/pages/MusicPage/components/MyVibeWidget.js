...
const LyricsCanvas = React.memo(
  ({ isPlaying, lyricsData, currentTime, dominantColor, currentTrack }) => {
    ...
    const filteredLines = useMemo(() => {
      if (
        ...
        const filtered = lyricsData.synced_lyrics
          .filter(
            line =>
              line && line.text !== undefined && line.text.trim().length > 0
          )
          .map((line, index) => ({
            ...line,
      ...
      time => {
        ...
        if (newLineIndex !== currentLineIndex && newLineIndex >= 0) {
          ...
          if (
            ...
            const rawWords = currentLine.text
              .split(' ')
            ...
            const newWordPositions = limitedWords.map((word, index) => {
              const angle =
                (index / limitedWords.length) * Math.PI * 2 - Math.PI / 2;
      ...
      const animate = currentTime => {
        ...
        if (
          ...
          (wordPositions.length > 0 ||
            fadeInWords.length > 0 ||
            fadeOutWords.length > 0)
        ) {
        ...
        if (isPlaying && wordPositions.length > 0) {
          const canvasWidth =
            canvas.width / (isMobile || isLowEnd ? 1 : window.devicePixelRatio);
          const canvasHeight =
            canvas.height /
            (isMobile || isLowEnd ? 1 : window.devicePixelRatio);

          const oldVisibleWords = oldWordPositions.filter(wordPos => {
            const fadeOutWord = fadeOutWords.find(
              fw => fw.word === wordPos.word
            );
            return fadeOutWord;
          });
...
const WaveContainer = React.memo(({ isPlaying }) => {
  ...
  useEffect(() => {
    ...
    const getWaveData = () => {
      ...
      const bass =
        Math.max(beat1 * 0.8, beat2 * 0.6) * progressIntensity + kickBeat * 0.4;
      const mid =
        Math.max(beat1 * 0.6, beat2 * 0.8, beat3 * 0.4) * progressIntensity +
        snareBeat * 0.3;
      const treble =
        Math.max(beat2 * 0.4, beat3 * 0.9) * progressIntensity +
        hihatBeat * 0.2;
      ...
      if (isPlaying) {
        ...
        waves.forEach((wave, waveIndex) => {
          ...
          const dynamicAmplitude =
            wave.amplitude * (0.4 + wave.intensity * 2.2);
          ...
          for (let x = 0; x <= canvasWidth; x += 0.5) {
            const wave1 =
              Math.sin(x * dynamicFreq + time * (0.8 + waveIndex * 0.2)) *
              dynamicAmplitude;
            const wave2 =
              Math.sin(
                x * dynamicFreq * 1.3 + time * (1.1 + waveIndex * 0.15)
              ) *
              (dynamicAmplitude * 0.4);
            const wave3 =
              Math.sin(x * dynamicFreq * 0.8 + time * (0.6 + waveIndex * 0.1)) *
              (dynamicAmplitude * 0.3);
...
const MyVibeWidget = ({ onClick, isPlaying, currentTrack, currentSection }) => {
  ...
  const { lyricsData, loadingLyrics, dominantColor, filteredLines } =
  ...
  return (
    ...
    >
      ...
      <div className={styles.contentOverlay}>
        <h2 className='font-bold text-lg' style={{ marginTop: '85px' }}>
