  ...
  const handleVibeClick = useCallback(async () => {
    ...
    try {
      ...
      if (
        ...
        setError(
          'Не удалось найти треки для вашего вайба. Попробуйте лайкнуть что-нибудь!'
        );
        setSnackbar({
          open: true,
          message:
      ...
      setSnackbar({
        open: true,
        message:
          'Не удалось загрузить плейлист "Мой Вайб". Попробуйте еще раз.',
        severity: 'error',
      });
    } finally {
      setIsLoading(false);
  ...
  React.useEffect(() => {
    if (trackId) {
      ...
      if (typeof playTrackById === 'function') {
        ...
        console.log(
          'User not authenticated, saving trackId to localStorage:',
          trackId
        );
