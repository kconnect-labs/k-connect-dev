...
const ProfilePreview: React.FC<ProfilePreviewProps> = ({ user, profileData }) => {
  ...
  return (
    <Paper sx={{
      p: 0,
      borderRadius: '16px',
      background: displayUser?.profile_id === 2 && displayUser?.banner_url
        ? `url(${displayUser.banner_url}), rgba(255, 255, 255, 0.03)`
        : 'rgba(255, 255, 255, 0.03)',
      backgroundSize: displayUser?.profile_id === 2 && displayUser?.banner_url ? 'cover' : undefined,
      backgroundPosition: displayUser?.profile_id === 2 && displayUser?.banner_url ? 'center' : undefined,
      backdropFilter: 'blur(20px)',
      WebkitBackdropFilter: 'blur(20px)',
      boxShadow: '0 5px 15px rgba(0, 0, 0, 0.2)',
      overflow: 'hidden',
      ...
      <Box sx={{ px: 2, pb: 2, pt: 0, mt: displayUser?.profile_id === 2 ? 1 : -4 }}>
        <Box sx={{ 
          display: 'flex', 
          ...
          <Box sx={{ position: 'relative' }}>
            <Avatar 
              ...
              sx={{ 
                width: 80, 
                height: 80, 
                border: (displayUser?.status_color && displayUser?.status_text && displayUser?.subscription) 
                  ...
                  : theme => theme.palette.mode === 'dark'
                      ? '3px solid #121212'
                      : '3px solid #ffffff',
                boxShadow: (displayUser?.status_color && displayUser?.status_text && displayUser.subscription) 
                  ? `0 0 12px ${displayUser.status_color}80` 
                  : displayUser?.subscription 
                    ? (displayUser.subscription.type === 'premium' ? '0 0 12px rgba(186, 104, 200, 0.5)' : displayUser.subscription.type === 'pick-me' ? '0 0 12px rgba(208, 188, 255, 0.5)' : displayUser.subscription.type === 'ultimate' ? '0 0 12px rgba(124, 77, 255, 0.5)' : '0 0 12px rgba(66, 165, 245, 0.5)') 
        ...
        <Box sx={{ whiteSpace: 'nowrap', mt: 1 }}>
          <Box sx={{ display: 'flex', alignItems: 'center' }}>
            <Typography 
              ...
              sx={{ 
                ...
                textShadow: displayUser?.profile_id === 2 ? '0 1px 3px rgba(0,0,0,0.7)' : 'none',
                background: displayUser?.profile_id === 2 ? 'none' : theme => theme.palette.mode === 'dark' 
                  ? 'linear-gradient(90deg, #fff 0%, rgba(255,255,255,0.8) 100%)'
                  : 'linear-gradient(90deg, #000 0%, rgba(0,0,0,0.8) 100%)',
                WebkitBackgroundClip: displayUser?.profile_id === 2 ? 'unset' : 'text',
                WebkitTextFillColor: displayUser?.profile_id === 2 ? 'unset' : 'transparent',
                fontSize: '1.1rem'
              }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: 0.5, flexWrap: 'wrap', maxWidth: '100%' }}>
            <Typography 
              ...
              sx={{ 
                fontWeight: 500,
                color: displayUser?.profile_id === 2 ? 'rgba(255,255,255,0.9)' : theme => theme.palette.text.secondary,
                textShadow: displayUser?.profile_id === 2 ? '0 1px 2px rgba(0,0,0,0.5)' : 'none',
                display: 'flex',
                alignItems: 'center',
                background: displayUser?.profile_id === 2 
                  ? 'rgba(0,0,0,0.3)'
                  : theme => theme.palette.mode === 'dark' ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.03)',
                px: 1,
                py: 0.3,
                borderRadius: 1,
                border: displayUser?.profile_id === 2
            ...
            {displayUser?.connect_info && displayUser.connect_info.length > 0 && (
              <>
                <LinkRoundedIcon 
                  sx={theme => ({ 
                    width: '1.5em',
                    height: '1.5em',
                    fontSize: 14,
                    color: displayUser?.profile_id === 2 ? 'rgba(255,255,255,0.9)' : theme.palette.text.secondary
                  })} 
                />
                <Typography
                  variant="body2"
