...
const StatusForm: React.FC<StatusFormProps> = ({
  ...
  const presetColors = [
    '#D0BCFF', '#90CAF9', '#A5D6A7', '#FFCC80', '#EF9A9A', 
    '#CE93D8', '#FFF59D', '#B0BEC5', '#F48FB1', '#81D4FA'
  ];
  ...
  useEffect(() => {
    console.log('StatusForm useEffect triggered with:', { profileData, subscription, isChannel });
    ...
    if (profileData) {
      ...
      else if (isChannel && profileData.user && profileData.user.main_account_id) {
      ...
      console.log('profileData.user?.status_text:', profileData.user?.status_text);
