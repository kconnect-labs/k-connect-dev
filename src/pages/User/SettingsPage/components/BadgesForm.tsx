...
const BadgesForm: React.FC<BadgesFormProps> = ({ onSuccess }) => {
  ...
  // Фильтрация бейджей
  const earnedBadges = userAchievements.filter(achievement => !achievement.image_path.includes('shop/'));
  const purchasedBadges = userAchievements.filter(achievement => achievement.image_path.includes('shop/'));
            ...
            <>
              ...
              <Grid container spacing={1.5}>
                {purchasedBadges.map((achievement) => (
                  <Grid item xs={4} sm={3} md={2.4} key={achievement.id}>
                    <Card elevation={0} sx={cardStyle(achievement.is_active, true)}>
                      {achievement.is_active && (
                        <Box sx={activeIndicatorStyle}>
                          <CheckIcon sx={{ fontSize: 12 }} />
                        </Box>
