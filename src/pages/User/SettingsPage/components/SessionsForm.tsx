...
const SessionsForm: React.FC<SessionsFormProps> = ({ onSuccess }) => {
  ...
  const [confirmDialog, setConfirmDialog] = useState({ open: false, sessionId: null as string | null });
    ...
    <>
      <Box sx={{ p: 2 }}>
        ...
        <Typography variant="body2" sx={{ color: 'text.secondary', mb: 3 }}>
          Управляйте устройствами, подключенными к вашему аккаунту
          (Временное меню - Мостовое)
        </Typography>
      ...
      >
        ...
        <DialogActions sx={{ borderTop: '1px solid #222' }}>
          ...
          <Button 
            onClick={() => confirmDialog.sessionId && handleDeleteSession(confirmDialog.sessionId)} 
            variant="contained"
            sx={{
              backgroundColor: 'rgba(208, 188, 255, 0.1)',
              color: '#d0bcff',
