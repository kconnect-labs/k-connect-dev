...
const CreatePost = ({
  ...
  const processFiles = files => {
    ...
    const imageFiles = validTypeFiles.filter(file =>
      ALLOWED_IMAGE_TYPES.includes(file.type)
    );
    const videoFiles = validTypeFiles.filter(file =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (error.response && error.response.status === 429) {
        ...
        setError(
          'Произошла ошибка при создании поста. Пожалуйста, попробуйте еще раз.'
        );
      }
    } finally {
  ...
  return (
    ...
    >
                    ...
                    <img
                      src={
                        track.cover_path.startsWith('/static/')
                          ? track.cover_path
                          : `/static/uploads/music/covers/${track.cover_path}`
                      }
                      alt={track.title}
                      style={{
                        width: '100%',
      ...
      <Snackbar
        ...
        onClose={() =>
          setMediaNotification({ ...mediaNotification, open: false })
        }
