...
const Login = () => {
    ...
    const checkAuthentication = async () => {
      try {
        const hasSessionCookie = document.cookie
          .split(';')
          .some(cookie => cookie.trim().startsWith('kconnect_session='));

        if (hasSessionCookie) {
          console.log(
            'Session cookie found, redirecting to saved path:',
            redirectPath
          );
        ...
        if (isAuthenticated) {
          console.log(
            'User already authenticated, redirecting to saved path:',
            redirectPath
          );
          navigate(redirectPath, { replace: true });
        }
    ...
    const handleTelegramAuthMessage = event => {
      if (event.data.type === 'telegram-auth-success') {
        ...
        try {
          ...
          setError(
            'Произошла ошибка при входе через Telegram. Пожалуйста, попробуйте снова.'
          );
        } finally {
          setLoading(false);
        }
  ...
  const handleTelegramLogin = () => {
    ...
    if (
      ...
      setError(
        'Не удалось открыть окно авторизации. Пожалуйста, проверьте настройки блокировки всплывающих окон в вашем браузере.'
      );
    }
  };
            ...
            <Box sx={{ textAlign: 'center' }}>
              ...
              >
                ...
                {banInfo.is_auto_ban && (
                  ...
                  >
                    Блокировка выдана автоматически за получение 3
                    предупреждений от модераторов.
                  </Typography>
                )}
              </Box>
            </Box>
