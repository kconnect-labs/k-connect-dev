...
export const useNitroApi = () => {
  ...
  const verifyUser = useCallback(async (userId: number, verificationLevel: number) => {
  ...
  const getUserAchievements = useCallback(async (userId: number) => {
    return makeRequest<{ achievements: Achievement[]; total: number }>(`/users/${userId}/achievements`);
  }, [makeRequest]);

  const giveAchievement = useCallback(async (userId: number, badgeTypeId: number) => {
    return makeRequest<ApiResponse<any>>(`/users/${userId}/give-achievement`, {
      method: 'POST',
  ...
  const getUserSubscriptions = useCallback(async (userId: number) => {
    return makeRequest<{ subscriptions: Subscription[] }>(`/users/${userId}/subscriptions`);
  }, [makeRequest]);

  const giveSubscription = useCallback(async (userId: number, subscriptionType: string) => {
    return makeRequest<ApiResponse<any>>(`/users/${userId}/give-subscription`, {
      method: 'POST',
  ...
  const getSubscriptionTypes = useCallback(async () => {
    return makeRequest<{ subscription_types: SubscriptionType[] }>('/subscription-types');
  }, [makeRequest]);

  
  const assignModerator = useCallback(async (userId: number, permissions: Partial<ModeratorPermission>) => {
  ...
  const getRedemptionKeys = useCallback(async (userId: number, params?: {
    page?: number;
    per_page?: number;
    active?: boolean;
    used?: boolean;
    ...
    return makeRequest<PaginationResponse<RedemptionKey>>(`/users/${userId}/keys${queryString ? `?${queryString}` : ''}`);
  }, [makeRequest]);

  const generateKeys = useCallback(async (userId: number, keyData: {
    ...
    return makeRequest<ApiResponse<{ keys: string[] }>>(`/users/${userId}/generate-keys`, {
      method: 'POST',
      data: keyData,
    });
  }, [makeRequest]);

  const getBugReports = useCallback(async (params?: {
    page?: number;
    per_page?: number;
    search?: string;
  }) => {
    ...
    const queryString = searchParams.toString();
    return makeRequest<{ bug_reports: any[]; total: number; page: number; has_next: boolean }>(`/bug-reports${queryString ? `?${queryString}` : ''}`);
  }, [makeRequest]);

  const updateBugReportStatus = useCallback(async (bugId: number, status: string) => {
  ...
  const deleteBugReport = useCallback(async (bugId: number) => {
    return makeRequest<ApiResponse<any>>(`/bug-reports/${bugId}`, {
      method: 'DELETE',
    });
  }, [makeRequest]);
  ...
  const getCommentsByPostId = useCallback(async (postId: number) => {
  ...
  const getBadges = useCallback(async (params?: {
    ...
    const queryString = searchParams.toString();
    return makeRequest<{ badges: ShopBadge[]; total: number; page: number; has_next: boolean }>(`/badges${queryString ? `?${queryString}` : ''}`);
  }, [makeRequest]);

  const getBadgeById = useCallback(async (badgeId: number) => {
    return makeRequest<ShopBadge>(`/badges/${badgeId}`);
  }, [makeRequest]);

  const updateBadge = useCallback(async (badgeId: number, badgeData: {
    name?: string;
    description?: string;
    price?: number;
    is_active?: boolean;
  ...
  const getTracks = useCallback(async (page: number = 1, search: string = '', trackId?: number, suspiciousOnly: boolean = false): Promise<PaginationResponse<Track>> => {
    try {
      setLoading(true);
  ...
  const clearTrackDescription = useCallback(async (trackId: number): Promise<void> => {
    try {
      setLoading(true);
  ...
  const clearTrackLyrics = useCallback(async (trackId: number): Promise<void> => {
    try {
      setLoading(true);
  ...
  const getArtists = useCallback(async (page: number = 1, search?: string): Promise<PaginationResponse<Artist>> => {
    try {
      setLoading(true);
      const params = new URLSearchParams({
        page: page.toString(),
  ...
  const createArtist = useCallback(async (artistData: FormData): Promise<ApiResponse<Artist>> => {
    try {
      setLoading(true);
  ...
  const updateArtist = useCallback(async (id: number, artistData: FormData | any): Promise<ApiResponse<Artist>> => {
    try {
      setLoading(true);
      const response = await axios.put(`${API_BASE}/artists/${id}`, artistData, {
        headers: artistData instanceof FormData ? {
          'Content-Type': 'multipart/form-data',
        } : {
          'Content-Type': 'application/json',
        },
  ...
  const deleteArtist = useCallback(async (id: number): Promise<ApiResponse<void>> => {
    try {
      setLoading(true);
  ...
  const getArtistTracks = useCallback(async (artistId: number): Promise<ApiResponse<any>> => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE}/artists/${artistId}/tracks`);
      return response.data;
    } catch (error) {
  ...
  const searchTracksForArtist = useCallback(async (artistName: string, exactMatch: boolean = false, limit: number = 50): Promise<ApiResponse<any>> => {
    try {
      setLoading(true);
  ...
  const assignTracksToArtist = useCallback(async (trackIds: number[], artistId: number): Promise<ApiResponse<void>> => {
    try {
      setLoading(true);
  ...
  const removeTrackFromArtist = useCallback(async (trackId: number, artistId: number): Promise<ApiResponse<void>> => {
    try {
      setLoading(true);
      const response = await axios.post(`${API_BASE}/artists/${artistId}/remove-track`, {
        track_id: trackId
      });
      return response.data;
    } catch (error) {
  ...
  const getUserWarnings = useCallback(async (userId: number) => {
    try {
      setLoading(true);
      const response = await axios.get(`${API_BASE}/warnings?user_id=${userId}`);
      return response.data;
    } catch (error: any) {
  ...
  const issueWarning = useCallback(async (userId: number, reason: string, details?: string) => {
    try {
      setLoading(true);
  ...
  const banUser = useCallback(async (userId: number, reason: string, durationDays: number, details?: string) => {
    try {
      setLoading(true);
      ...
      setError(error.response?.data?.error || 'Ошибка блокировки пользователя');
      throw error;
    } finally {
      setLoading(false);
    }
  ...
  const unbanUser = useCallback(async (banId: number) => {
    try {
      ...
      setError(error.response?.data?.error || 'Ошибка разблокировки пользователя');
      throw error;
    } finally {
      setLoading(false);
    }
