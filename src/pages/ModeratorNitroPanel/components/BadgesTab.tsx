...
const BadgesTab: React.FC = () => {
  ...
  const { getBadges, updateBadge, deleteBadge, loading, error, clearError } = useNitroApi();
  ...
  const fetchBadges = useCallback(async (page = 1, append = false) => {
      ...
      if (response.badges) {
        ...
        setBadges(prev => {
          ...
          if (append) {
            ...
            return [...prev, ...newBadges.filter((b: ShopBadge) => !ids.has(b.id))];
          }
          return newBadges;
        });
      }
  ...
  }, [badgesHasNext, badgesLoading, badgesLoadingMore, badgesPage, fetchBadges]);
    ...
    return (
      <Box sx={{ p: 3, textAlign: 'center' }}>
        <Alert severity="error" sx={{ maxWidth: 600, mx: 'auto' }}>
          ...
          <Typography>
            У вас нет прав на управление бейджами
          </Typography>
        </Alert>
      </Box>
    );
  ...
  return (
    <Box>
      ...
      {badgesError && (
        <Alert severity="error" sx={{ mb: 1 }} onClose={() => setBadgesError(null)}>
          {badgesError}
        </Alert>
      )}
                    ...
                    <Box sx={{ flex: 1 }}>
                      ...
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                        <Chip
                          ...
                          icon={badge.is_active ? <Visibility /> : <VisibilityOff />}
                        />
        ...
        <DialogContent sx={{ pt: 1 }}>
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
            ...
            <FormControlLabel
              control={
                <Switch
                  ...
                  onChange={(e) => setEditForm(prev => ({ ...prev, upgrade: e.target.checked }))}
                />
              }
              label="Upgrade"
            />
            <TextField
              ...
              onChange={(e) => setEditForm(prev => ({ ...prev, color_upgrade: e.target.value }))}
              size="small"
              inputProps={{
                pattern: "^#[0-9A-Fa-f]{6}$",
